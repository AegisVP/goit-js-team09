...
function openModalDev(e, modalDev) {
  ...
  document.querySelector('.developer__grid').addEventListener('click', e => {
		if (e.target.dataset.name && e.target.dataset.name !== 'run') {
			goitEl.classList.add('visually-hidden');
			teamEl.classList.add('visually-hidden');
			teamText.innerHTML = `<p class="dev-name">Hi, I am ${e.target.dataset.name}!</p><p>Nice to meet you!</>`;
		...
		if (e.target.dataset.name === 'run') {
			runAnimation(goitEl, teamEl, teamText);
		}
